...
go
CREATE TRIGGER TRG_DG
ON DOCGIA
AFTER INSERT
AS
DECLARE @MADOCGIA INT = 0;
BEGIN
	SELECT @MADOCGIA = INSERTED.MADOCGIA FROM INSERTED;
	UPDATE DOCGIA
	SET NGDENHAN = DATEADD(month, 4, NGLAPTHE)
	WHERE MADOCGIA = @MADOCGIA;
END;

DROP PROCEDURE IF EXISTS UPDATENGDENHAN; 
GO
CREATE PROC UPDATENGDENHAN (@madocgia int)
AS
SET NOCOUNT ON
BEGIN
	UPDATE DOCGIA
	SET NGDENHAN = DATEADD(month, 4, NGDENHAN)
	WHERE MADOCGIA = @madocgia
END;
